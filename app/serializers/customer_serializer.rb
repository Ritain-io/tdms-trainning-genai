...
class CustomerSerializer < ActiveModel::Serializer
  attributes :id, :full_name, :environment, :document_type, :document_value, :customer_type, :state,
  ...
  def audit_logs
    if action == 'show'
      versions = Version.where(item_type: 'Customer', item_id: object.id).order(created_at: :desc)[0..6]
